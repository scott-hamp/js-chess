...
function getOpeningWithMoves(movesNotation)
{
	...
	for(var i = 0; i < _openings.length; i++)
	{
		...
		if(matchingCount <= openingMovesMatchingCount) continue;
...
{
	var lines = _openingsRawData.split("\n");

	for(var i = 0; i < lines.length; i++)
	{
		var ECOCode = lines[i].substring(0, 3);
		var name = lines[i].substring(4);

		var movesNotationLine = lines[i + 1].replace("  ", " ").trim();
		var movesNotationLineParts = movesNotationLine.split(" ");
		var movesNotation = [];
		for(var j = 1; j < movesNotationLineParts.length; j++)
		{
			movesNotation.push(movesNotationLineParts[j]);
			if(movesNotationLineParts[j + 1] == null) break;
			movesNotation.push(movesNotationLineParts[j + 1]);
			j += 2;
		}
...
{
	var filterStr = "";
	if(filter != null) filterStr = filter.toLowerCase();
	...
	for(var i = 0; i < _openings.length; i++)
	{
		...
		{
			if(!opening.name.toLowerCase().includes(filterStr)) continue;
		}

		innerHTML += `<option value="opening_${i}">(${opening.ECOCode}) ${opening.name}</option>`;
	}
