...
function loadOpenings()
{
	...
	{
		var ECOCode = lines[i].replace("  ", " ").trim();
		var name = lines[i + 1].replace("  ", " ").trim();

		var movesNotationLine = lines[i + 2];
		for(var j = 1; j < 100; j++) 
			movesNotationLine = movesNotationLine.replace(`${j}. `, "");
		movesNotationLine = movesNotationLine.replace("  ", " ").trim();
