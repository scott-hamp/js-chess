...
function getOpeningWithMoves(moves)
{
	...
	for(var i = 0; i < _openings.length; i++)
	{
		...
		{
			result = opening;
			resultMovesMatchingCount = matchingCount;
			continue;
		}

		if(matchingCount <= resultMovesMatchingCount) continue;
...
function loadOpenings()
{
	...
	for(var i = 0; i < lines.length; i++)
	{
		...
		var codeNameLine = lines[i];
		var code = codeNameLine.substring(0, 3);
		var name = codeNameLine.substring(4).trim();

		var movesLine = lines[i + 1].replace("  ", " ").trim();
		for(var n = 1; n < 100; n++)
